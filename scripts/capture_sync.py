...
def list_ifaces() -> list[dict]:
    ...
    for iface in conf.ifaces.values():
        ...
        if not name:
            continue
        detail = str(getattr(iface, "description", "") or "").strip()
...
def _consume(source: str, progress, *, iface: str | None = None, seconds: int = 120, pcap: str | None = None, mark_missing: bool = False) -> dict:
    ...
    if mark_missing and not complete:
        ...
        mark_missing = False
    elif not mark_missing:
        report("回放只更新出现过的精灵", len(records), collector.total_page or len(records))
    return upsert_pets(records, mark_missing=mark_missing, progress=progress)


def sync_from_live(iface: str, seconds: int = 120, progress=None) -> dict:
    if not iface:
