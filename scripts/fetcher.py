...
def main():
    ...
    if user_info:
        ...
        print("Warning: Failed to get user info. Check tokens.")

    # Show refresh time
    refresh_info = fetch_refresh_time()
    if refresh_info:
        next_refresh = refresh_info.get('next_auto_refresh_time', 0)
        print(f"Pet refresh time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(next_refresh))}")

    # 同步蛋组数据 (从 docs/egg.json)
    try:
        with open('docs/egg.json', 'r', encoding='utf-8') as f:
            egg_data = json.load(f)
            for item in egg_data:
                # itemId 对应 egg.json 里的 id
                cursor.execute("UPDATE pet_base_info SET egg_groups = ? WHERE itemId = ?", 
                               (json.dumps(item.get("eggGroups", []), ensure_ascii=False), item["id"]))
        conn.commit()
        print("Egg groups synchronized from egg.json")
    except Exception as e:
        print(f"Warning: Failed to sync egg groups: {e}")

    # 同步基础配置数据 (从 docs/PETBASE_CONF.json)
    try:
        with open('docs/PETBASE_CONF.json', 'r', encoding='utf-8') as f:
        ...
        conn.commit()
        print("Base info synchronized from PETBASE_CONF.json")
    except Exception as e:
        print(f"Warning: Failed to sync from PETBASE_CONF.json: {e}")
    
    current_page = 1
    page_size = 100
    all_api_pets = [] # Store (serial_num, base_id, talent_rank)
    all_api_serials = set()
    
    print("Fetching all pets from list API...")
    while True:
    ...
    print(f"Found {len(all_api_serials)} pets in total from API.")
        ...
        if not cursor.fetchone():
            ...
            if base_info:
                ...
                """, (
                    ...
                    json.dumps(evolution_ids, ensure_ascii=False) # 同样存为 JSON 数组
                ))
                conn.commit()
                time.sleep(0.5)
    ...
    if released_serials:
        print(f"Detected {len(released_serials)} released pets. Marking as inactive...")
    ...
    for pet in all_api_pets:
        ...
        if row:
            # If pet exists, check if it needs detail update (e.g. medal is NULL)
            ...
            if row["height"] is None or row["catch_ball"] is None:
                print(f"Pet {sn} missing details. Fetching...")
        ...
        print(f"New pet detected: {sn}. Fetching detail...")
        detail = gateway_request("/api/pet/detail", {"id": str(sn)})
        if detail:
            cursor.execute("""
    ...
    print(f"Fetch completed. {new_count} new pets added, {updated_count} existing pets updated with details.")
    conn.close()
